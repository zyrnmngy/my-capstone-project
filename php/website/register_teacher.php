...
$username = 'u769346877_filibustero'; // Update if needed
$password = 'Filibustero_capstone08'; // Update if needed
...
function registerTeacher($pdo) {
    ...
    // Log processed data
    error_log("Processed data - ID: $idNumber, Name: $fullName, Username: $username, Sections: " . print_r($sections, true));
    ...
    if (empty($idNumber) || empty($fullName) || empty($username) || empty($password)) {
    ...
    try {
        ...
        // Insert into users table with teacher type
        $sql = "INSERT INTO users (user_type, id_number, full_name, username, password_hash, temp_password, created_at, updated_at) 
                VALUES ('teacher', :id_number, :full_name, :username, :password_hash, 0, NOW(), NOW())";
        
        error_log("Executing users insert query: $sql");
        error_log("With parameters: ID=$idNumber, Name=$fullName, Username=$username");
        ...
        $verifyStmt = $pdo->prepare("SELECT id, username, user_type FROM users WHERE id = :id");
        ...
        $finalCheck = $pdo->prepare("SELECT u.id, u.username, u.user_type, t.is_active FROM users u JOIN teachers t ON u.id = t.user_id WHERE u.id = :id");
...
// Optional: Add a function to handle GET requests for getting sections
